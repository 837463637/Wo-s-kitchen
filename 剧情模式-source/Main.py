...
if begins == 7:
    ...
    input("输入：handoff(wood)前往树林\n")
    items2 = list(range(0, 57))  # 假设有57个项
    l2 = len(items2)
    for i, item in enumerate(items2):
        # 模拟一些耗时的操作
        time.sleep(0.3)
        # 更新进度条
        progress_bar(i + 1, l2, prefix='寻找中:', suffix='完成', length=100)
    dh_W("好了！")
    S()
    dh_W("一扔？")
    S()
    ...
    dh_W("你好！")
    S()
    dh_HSDL("哎呀！我的[苹果]！Miamiamia")
    S()
    dh_W("你好！")
    S()
    dh_HSDL("噢！不好意思！吃得太起劲了，真是饿死我了。")
    S()
    dh_W("我买一个树卵")
    S()
    dh_HSDL("不过，看在你喂我[苹果]的份上，这个送你吧[葡萄]。")
    S()
    print(f"你的背包\n￥：{Money_Plot}\n1.葡萄 2.无 3.无 4.无\n")
    S()
    dh_W("谢谢！")
    S()
    dh_HSDL("就是下次别再把[苹果]往地上丢了，多脏啊")
    SJ(2.0)
    dh_W("要水龙头吧？")
    S()
    input("输入buy(tap)购买水龙头\n")
    S()
    buy_plot(5)
    dh_W("OK!")
    items3 = list(range(0, 50))
    l3 = len(items3)
    for i, item in enumerate(items3):
        # 模拟一些耗时的操作
        time.sleep(0.1)
        # 更新进度条
        progress_bar(i + 1, l3, prefix='做菜中:', suffix='完成', length=100)
    dh_W("[葡萄酒]做好啦！")
    S()
    dh_MTY("真棒！这个味道浓郁香醇，葡萄的口感伴随着水润的质地，仿佛能洗刷灵魂的污垢，简直就是世界上最好喝的东西！")
    S()
    dh_MTY("我结账咯 ￥9")
    S()
    dh_W("哇？这个值9元？你是怎么知道它应该定多少钱的？")
    S()
    dh_MTY("hoho～我是博学的猫头鹰嘛，定价我当然知道啦")
    S()
    dh_W("那你可以帮帮我吗？我都开餐厅了，但是对于菜品定价毫无头绪")
    S()
    dh_MTY("没问题～反正我也闲着，我就帮你定定价吧，不懂的随时问我")
    WWDX()
    Money_Plot = Money_Plot + 9
    Money_Plot_detection(19)
    WWDX()
